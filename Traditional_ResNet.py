...
class ResNet(nn.Module):
    ...
    def __init__(self):
        ...
        self.resn1 = nn.Sequential(             # image: 16x16 input
        ...
        self.resn2 = nn.Sequential(             # image: 16x16 input
        ...
        self.proj2 = nn.Sequential(             # image: 16x16 input
            nn.Conv2d(256, 512, 1, 1, bias=False),
            nn.BatchNorm2d(512)
        )                                       # image: 16x16 output
        
        self.resn3 = nn.Sequential(             # image: 16x16 input
        ...
        self.avgpool = nn.AvgPool2d(2)          # image: 8x8 output
    ...
    def forward(self, x):
        ...
        x = self.resn1(x) + x # residual block
        x = F.relu(x)
        x = self.resn2(x) + self.proj1(x) # residual block with skip projection
        x = F.relu(x)
        x = self.resn3(x) + self.proj2(x) # residual block with skip projection
        x = F.relu(x)
...
def load(resnet, test_loader, criterion, device):
    ...
    try:
        resnet.load_state_dict(torch.load('./models/resnet_trad.pth'))
        print("Done!")
        if args.cm:
...
def save(resnet, test_loader, criterion, device, best_loss, best_params):
    ...
    if (best_loss < saved_loss):
        torch.save(best_params,"./models/resnet_trad.pth") # Saves best model
        print("Model saved to models/resnet_trad.pth") 
    else:
...
def predictImage(resnet, device):
    ...
    try:
        ...
        resnet.load_state_dict(torch.load('./models/resnet_trad.pth')) # Load the saved model
