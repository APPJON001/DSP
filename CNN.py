...
class CNN(nn.Module):
    ...
    def __init__(self):
        ...
        self.conv1 = nn.Conv2d(1, 32, 3, padding=1, bias=False) # Convolution 1 (bias set to False due to batchnorm)
        ...
        self.fc1 = nn.Linear(512*2*2,1024) # first hidden layer after feature extraction
        ...
        self.fc3 = nn.Linear(512, 15) # third hidden layer
...
def load(cnn, test_loader, criterion, device):
    ...
    try:
        ...
        print("Done!")
        if args.cm:
            test_acc, test_loss, cm = conf_matrix(cnn, test_loader, criterion, device)
            ...
            test_acc, test_loss = test(cnn, test_loader, criterion, device)
            print(f"Test accuracy = {test_acc*100:.2f}%, Test loss = {test_loss:.4f}")
...
def save(cnn, test_loader, criterion, device, best_loss, best_params):
    ...
    if (best_loss < saved_loss):
        torch.save(best_params,"./models/cnn.pth") # Saves best model
...
def predictImage(cnn, device):
    ...
    try:
        ...
        cnn.load_state_dict(torch.load('./models/cnn.pth')) # Load the saved model
...
def main():
    ...
    if args.img: predictImage(cnn, device)
    ...
    # Define optimizer
    optimizer = optim.Adam(cnn.parameters(), lr=LEARNING_RATE, weight_decay=WEIGHT_DECAY) # Gets up to 97.8%
